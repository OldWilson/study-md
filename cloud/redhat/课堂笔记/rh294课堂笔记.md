...
3、本地培训环境，开启classroom，bastion后在，ssh root@bastion后，通过yum安装。（也可以在workstation上安装，serverX不能安装）
【bastion】
...
【重要】ansible --version 查看ansible的版本及当前正在应用的配置文件
...
工具 还是 服务？ 
ansible可以做什么？批量部署：安装程序，管理文件
ansible管理部署和ssh部署区别：幂等性
ansible和其他自动化工具，学习成本低，ssh，不用安装客户端，提供了安全性
```
...
student ----yes--student----- student   sudo -i  root
...
[student@bastion ansible]$ ansible share --list -i inventory
[student@bastion ansible]$ ansible-inventory --graph -i inventory
```
...
[student@bastion ansible]$ cp inventory inventory1    #额外创建一份为inventory1
[student@bastion ansible]$ ls
ansible.cfg  inventory  inventory1
[student@bastion ansible]$ echo servere > inventory1  #指定inventory1内主机 为servere
[student@bastion ansible]$ mkdir invdir         	  #创建存放清单文件夹
[student@bastion ansible]$ ls
ansible.cfg  invdir  inventory  inventory1
...
├── inventory
└── inventory1
[student@bastion ansible]$ ansible-inventory --graph -i invdir  #检测 -i invdir指定清单目录
@all:
  |--@share:
  |  |--@dbservers:
  |  |  |--172.25.250.12
  ...
  |--@ungrouped:
  |  |--servera
  |  |--servere   	#发现可以看到两个清单的所有主机


#练习即可，然后恢复之前的样子，删除多清单配置，保留之前的inventory即可。
[student@bastion ansible]$ ls
ansible.cfg  invdir
[student@bastion ansible]$ mv invdir/inventory .
[student@bastion ansible]$ ls
ansible.cfg  invdir  inventory
[student@bastion ansible]$ rm -rf invdir
[student@bastion ansible]$ ls
ansible.cfg  inventory

```

### 2.6 将清单路径记录到ansible.cfg配置文件中，达到系统自动寻找调用的目的。
...
## 3. 执行一些临时命令，指定远程用户及密码

```bash
临时命令
1. 什么是Ad-hoc
称为临时命令，简单说，就是在命令行界面，直接通过一条ansible命令，去指定主机执行指定指令，功能有限，主要用于临时部署，临时查询，一些简单的操作。

例如： 
ansible pattern -m 模块名称 -a 模块参数 

[student@bastion ansible]$ ansible servera -m ping  #直接报错，发现没有权限，因为用户权限不足无法送模块指令

解决： 复习ssh远程时的过程
server======= client
student	-----  student
1、yes (host_key_checking免yes)
2、远程登陆需要密码
3、sudo也需要密码
4、指定远程用户
...
BECOME password[defaults to SSH password]: studnet  #切换sudo时的特权密码
servera | SUCCESS => {
    "ansible_facts": {
...
在配置文件中指定远程用户和清单文件中指定远程密码
[student@bastion ansible]$ grep -B 3 remote_user ansible.cfg 
[defaults]
inventory      = /home/student/ansible/inventory
host_key_checking = False
remote_user = root     #指定远程用户固定为root
[student@bastion ansible]$ grep -A 1 vars inventory 
[all:vars]   		   #指定组变量：all代表所有，vars代表设置组变量，vars写法是固定的
ansible_password=redhat     #在下一行设置组变量ansible_password代表远程用户密码，redhat是密码
[student@bastion ansible]$ ansible servera -m command -a 'id tom'  #再次执行发现不用添加额外的选项，此时是以root身份远程的。
...
servera | CHANGED | rc=0 >>
uid=1004(bob) gid=1004(bob) groups=1004(bob)


使用ansible Ad-hoc命令部署yum仓库
[student@bastion ~]$ ansible-doc -l | grep yum
[student@bastion ~]$ ansible-doc yum_repository    /EXAMPLE

ansible servera -m yum_repository -a 'name=BaseOS description=BaseOS baseurl=http://content.example.com/rhel8.4/x86_64/dvd/BaseOS enabled=yes gpgcheck=no'
ansible servera -m yum_repository -a 'name=AppStream description=AppStream baseurl=http://content.example.com/rhel8.4/x86_64/dvd/AppStream enabled=yes gpgcheck=no'
ansible servera -a 'yum repolist all'
ansible servera -a 'yum install -y httpd'
ansible servera -m shell -a 'rpm -qa | grep httpd'


[student@bastion ansible]$ ansible servera -m yum_repository -a '\
> name=AppStream \
