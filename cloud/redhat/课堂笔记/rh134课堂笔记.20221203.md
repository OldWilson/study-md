...
[root@servera /]# setfacl -m u:harry:rwx 
...
#### 五 security context介绍（这里的指令只为了帮助大家了解概念）
...
- 通过 `Z` 选项，Podman 自动将 SELinux `container_file_t context` 应用到主机目录
  ...
  ```bash
  方法一：（mysql相关）：
  [student@servera /]$ mkdir /home/student/dbdir/
  [student@servera /]$ chmod a+w /home/student/dbdir/
  [student@servera /]$ 
