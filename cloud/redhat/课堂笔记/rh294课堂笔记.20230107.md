...
### 2.2 在PLAYBOOK中定义和使用变量（play范围，有限度中）
...
- name: haha
  ...
  - package: httpd   #键：值    关系用：冒号隔开，冒号后有一个空格
  - ansible_user: root
...
2.用vars_files字段加载变量文件 
[greg@control ansible]$ cat var.yml   #制作变量文件
---
service_name: http   #自定义的变量键值关系

[greg@control ansible]$ vim web.yml
---
- name: haha
  hosts: node1
  vars:
  ...
  - name:
    firewalld:
      service: "{{ service_name }}"   #调用变量
      permanent: yes
      state: enabled
      immediate: yes
```

### 2.3 清单中定义变量（主机范围，有限度最低）
...
```bash
[greg@control ansible]$ ansible-vault --help
...
[greg@control ansible]$ ansible-vault create sec1.txt   #新建加密文件
New Vault password: 123456
Confirm New Vault password: 123456
[greg@control ansible]$  ansible-vault view sec1.txt    #查看
Vault password: 123456
a: 123
[greg@control ansible]$ ansible-vault edit sec1.txt     #编辑
Vault password: 123456
[greg@control ansible]$ echo heihei > sec2.txt          #创建变量文件
[greg@control ansible]$ ansible-vault  encrypt sec2.txt #加密现有文件
New Vault password: 123456
Confirm New Vault password: 123456
[greg@control ansible]$ cat sec2.txt  
[greg@control ansible]$ ansible-vault decrypt sec2.txt  #解密
Vault password: 123456
[greg@control ansible]$ cat sec1.txt 
...
3662
[greg@control ansible]$ ansible-vault rekey sec1.txt   #改密码
Vault password: 123456
New Vault password: redhat
Confirm New Vault password: redhat
[greg@control ansible]$ ansible-vault view sec1.txt 
Vault password: redhat
passwd: haha

`密码文件`
[greg@control ansible]$ echo redhat > secret.txt  #创建密码文件
[greg@control ansible]$ ansible-vault view sec1.txt --help
  --vault-id VAULT_IDS  the vault identity to use
  --vault-password-file VAULT_PASSWORD_FILES
[greg@control ansible]$ ansible-vault view sec1.txt --vault-password-file=secret.txt
passwd: haha
[greg@control ansible]$ ansible-vault view sec1.txt --vault-id=secret.txt  #通过变量文件指定密码
passwd: haha

`密码文件记录到ansible.cfg配置文件中`
...
### 3 常用when条件语句(重点，带default变量查询方法)
...
### 4 （block、rescue、always）定义区域中失败的任务
...
[greg@control ansible]$ cat test.yml   (考)
---
- name: haha
  hosts: node1
  tasks:
...
```bash
第一个tasks任务
[greg@bastion ansible]$ mkdir tasks
...
- name:
  yum:
    ...
    state: latest
第二个tasks任务
[greg@bastion ansible]$ cat tasks/service.yml 
- name:
  service:
    name: httpd
    enabled: yes
...
```bash
`自定义角色流程
1.配置ansible.cfg文件中的roles-path(就是在决定角色存在系统什么位置，在创建或读使用角色时会使用该路径)
2.创建角色
3.完善角色功能（配置角色具体的功能，他们是由模块化目录组成）
4.使用角色
添加控制执行顺序（可选）
给角色使用变量（可选）
```
...
默认ansible会在roles子目录中查找角色。可以将自己的角色安装在~/ansible/roles子目录中。

1.默认路径：
[greg@control /]$ ansible-galaxy role --help   查看角色的子命令帮助
[greg@control /]$ ansible-galaxy list
# /usr/share/ansible/roles   -- 系统角色
# /etc/ansible/roles         -- 全局角色路径
[WARNING]: - the configured path /home/greg/.ansible/roles does not exist.  --默认该目录不存在，需要的话可以根据需求创建
`如果ansible无法找到该位置角色，会按照ansible.cfg中roles_path指定的目录中查找`

2、如果自定义工作目录情况下，我们可以自定义roles目录，并使用ansible.cfg中roles_path字段功能指定角色路径
[greg@control ansible]$ pwd      #进入自己的工作目录
/home/greg/ansible
[greg@control ansible]$ mkdir roles    #创建存放角色的目录
[greg@control ansible]$ vim ansible.cfg    
roles_path    = /home/greg/ansible/roles   #指定角色路径
[greg@bastion ansible]$ ansible-galaxy list   #一定要在工作目录中使用查看命令，调用当前工作目录配置文件中的角色路径 
# /home/greg/ansible/roles      #查询结果应和配置文件的roles_path字段一致
```

```bash
二 创建角色
[greg@control ansible]$ ansible-galaxy init roles/apache   #创建角色
- Role roles/apache was created successfully   #提示创建成功
[greg@control ansible]$ ansible-galaxy list    #列出所有角色
...
```bash
三、完善角色功能
[greg@control ansible]$  tree roles/apache (可选，方便查看路径信息)
[greg@control ansible]$  vim roles/apache/tasks/main.yml   #在tasks目录中完善角色内容
---
...
- name: create web page
  template:
    src: jin.j2    #jin.j2需要手动创建
    dest: /var/www/html/index.html
  notify:
  - restart http1   #需要时将调用handlers处理程序，需要手动创建，restart http1通知中的处理程序名要和handlers/main.yml中处理程序描述一致
...
[greg@control ansible]$ vim roles/apache/templates/jin.j2  #编写模板文件
ipadd={{ ansible_default_ipv4.address }}  hostname={{ ansible_hostname }}

[greg@control ansible]$ vim roles/apache/handlers/main.yml   #编写处理程序
---
- name: restart http1
  service:
    name: httpd
...
[greg@control ansible]$ curl node1
ipadd=172.25.250.9  hostname=node1
```
...
```yaml
  ...
  - name: 将lvm文件系统挂载到/mnt/dir1上
    mount:
      ...
      src: /dev/vg100/lv100
      fstype: xfs
      opts: defaults   #可以不写，默认为defaults
      state: mounted
