...
| node1、node2     |                                  | 1        | student:student | root:flectrag |
...
2、配置vim /etc/rsyslog.conf  添加  *.debug     /var/log/messages.debug
3、重启rsyslog日志服务
...
mandb
man -k journal
man journald.conf  ， Storage=
man systemd-journald | grep -A 1 mkdir
           mkdir -p /var/log/journal
           systemd-tmpfiles --create --prefix /var/log/journal
[root@servera ]# mkdir -p /var/log/journal           
[root@servera ]# systemd-tmpfiles --create --prefix /var/log/journal
[root@servera ]# systemctl restart systemd-journald
[root@servera ]# ll /var/log/journal
```
...
```bash
[kiosk@foundation0 Packages]# su - root
...
[root@foundation0 Packages]# rpm -q telnet
telnet-0.17-73.el8.x86_64
[root@foundation0 Packages]# rpm -e telnet  或者 rpm -e telnet-0.17-73.el8.x86_64
...
```bash
cd /content/rhel8.4/x86_64/dvd/AppStream/Packages
ls telnet
ls telnet*
rpm -ivh telnet-0.17-76.el8.x86_64.rpm
rpm -q telnet
rpm -qa
rpm -qa  | wc -l
rpm -qa  | grep telnet
...
rpm -ql telnet
rpm -qa | grep httpd
rpm -ql httpd-2.4.37-10.module+el8+2764+7127e69e.x86_64 | grep service
rpm -qc httpd-2.4.37-10.module+el8+2764+7127e69e.x86_64
...
rpm -qf /usr/bin/tree
rpm -q httpd
rpm -qi httpd-2.4.37-10.module+el8+2764+7127e69e.x86_64
rpm -q telnet
rpm -e telnet
rpm -q telnet
```
...
[root@foundation0 ~]# cd /etc/yum.repos.d/
[root@foundation0 yum.repos.d]# ls
redhat.repo  rhel-dvd.repo  rht-ucf.repo
[root@foundation0 yum.repos.d]# mkdir old
[root@foundation0 yum.repos.d]# mv * old/
mv: cannot move 'old' to a subdirectory of itself, 'old/old'
[root@foundation0 yum.repos.d]# ls
old
[root@foundation0 yum.repos.d]# man 5 yum.conf
[root@foundation0 yum.repos.d]# vim rhel.repo    #yum源文件名字自定义，结尾一定.repo
[BaseOS]
name=BaseOS
...
[root@foundation0 yum.repos.d]# yum repolist all
```
...
[foundation0]
查找utils软件包：
[root@foundation0]# yum provides yum-config-manager  注意：8.0版本dnf-utils，8.4版本yum-utils
[root@foundation0]# yum install -y dnf-utils-4.0.2.2-3.el8.noarch 

`如果yum源链接是http开头，需要用浏览器打开后搜索软件包`
rpm -ivh xxxxx/dnf-utils-4.0.2.2-3.el8.noarch.rpm
...
echo 123 > sou_file.txt
cat sou_file.txt 
ln /sou_file.txt /opt/link_file.txt
取消链接

[root@servera opt]# ls
home  link_file.txt  natasha  student
[root@servera opt]# unlink /opt/home
[root@servera opt]# ls
link_file.txt  natasha  student
```
